...
<form:form encType="multipart/form-data" class="wufoo leftLabel page" action="updatevehiclelogs.htm" method="GET" commandName="vehicleLogs">
...
<div id="purposeDropdownDiv">
	...
	<option value="0" label="Select">Select</option>
		...
		<c:if test="${visitorPurposeListVar.purpose==vehicleLogs.purpose}">
...
<input type="text" id="datefrom" name="datefrom" 
	value="${fn:split(vehicleLogs.inTime,' ')[0]}" readonly="readonly" class="field text medium"/>
	<img
...
<c:choose>
<c:when test="${fn:split(vehicleLogs.inTime,' ')[1]!='0'}">
<form:option value="${fn:split(vehicleLogs.inTime,' ')[1]}" label="${fn:split(vehicleLogs.inTime,' ')[1]}"/>
</c:when>
<c:otherwise>
...
<input type="text" id="dateTo" name="dateTo" 
	value="${fn:split(vehicleLogs.outTime,' ')[0]}" readonly="readonly" class="field text medium"/>
	<img
...
<c:choose>
<c:when test="${fn:split(vehicleLogs.inTime,' ')[1]!='0'}">
<form:option value="${fn:split(vehicleLogs.outTime,' ')[1]}" label="${fn:split(vehicleLogs.outTime,' ')[1]}"/>
</c:when>
<c:otherwise>
<form:option value="0" label="Select"/>
</c:otherwise>
