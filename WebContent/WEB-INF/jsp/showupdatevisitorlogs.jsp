...
<form:form encType="multipart/form-data" class="wufoo leftLabel page" action="updatevisitorlogs.htm" method="GET" commandName="visitorLogs">
<form:hidden path="logId"/>
<input type="hidden" name="oldVisitorId" value="${visitorLogs.visitorId}"/>
<ul>
<li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">To meet<span id="req_23" class="req">*</span></label>
<div>
<form:select path="employeeId" id="employeeId" class="field select medium">
...
<li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">Visitor Name<span id="req_23" class="req">*</span></label>
<div>
<form:select path="visitorId" id="visitorId" class="field select medium" disabled="true">
<form:option value="0">Select</form:option>
<c:forEach items="${visitorsList}" var="visitorsList">
    <form:option value="${visitorsList.visitorId}" label="${visitorsList.visitorName}">${visitorsList.visitorName}</form:option>
</c:forEach>
</form:select>
<form:errors path="visitorId" cssClass="error"></form:errors>
</div>
</li>
...
<%-- <li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">Gate No.</label>
<div>
	<form:input path="gateNo" id="gateNo" name="gateNo" class="field text medium"/>
	<form:errors path="gateNo" cssClass="error"></form:errors>
</div>
	
</li>

<li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">Purpose</label>
<div>
...
<div id="gateDropdownDiv">
	...
	<option value="0" label="Select">Select</option>
		...
		<c:url var="selectedGate" value=""></c:url>
		<c:if test="${visitorGatesListVar.visitorGates==visitorLogs.gateNo}">
		<c:url var="selectedGate" value="selected"></c:url>
		</c:if>
		<option value="${visitorGatesListVar.visitorGates}" selected="${selectedGate}">${visitorGatesListVar.visitorGates}</option>
		</c:forEach>
	</select>
...
<div id="purposeDropdownDiv">
	...
	<option value="0" label="Select">Select</option>
		...
		<c:url var="selectedPurpose" value=""></c:url>
		<c:if test="${visitorPurposeListVar.purpose==visitorLogs.purpose}">
		<c:url var="selectedPurpose" value="selected"></c:url>
		</c:if>
		<option value="${visitorPurposeListVar.purpose}" selected="${selectedPurpose}">${visitorPurposeListVar.purpose}</option>
		</c:forEach>
	</select>
	<form:errors path="purpose" cssClass="error"></form:errors>
...
<!-- 	<input type="button" value="Add" class="btTxt submit" onclick="saveTextBoxVisitorType();"> -->
	...
	<form:hidden path="purpose" id="purpose" name="purpose" class="field text medium"/>
	<form:errors path="purpose" cssClass="error"></form:errors>
...
<div>
<form:select path="inTime" class="field select medium">
...
<div>
<form:select path="outTime" class="field select medium">
<form:option value="0" label="Select" />
<c:forEach items="${calTimesList}" var="calTimesListVar" varStatus="status">
<form:option value="${calTimesListVar.time}" label="${calTimesListVar.time}" />
</c:forEach>
...
<li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">Vehicle</label>
<div>
	<form:input path="vehicle" id="vehicle" name="vehicle" class="field text medium"/>
	<form:errors path="vehicle" cssClass="error"></form:errors>
</div>
</li>

<li id="foli110" class="notranslate"><label class="desc" id="title110" for="Field110">Vehicle No.</label>
<div>
	<form:input path="vehicleNo" id="vehicleNo" name="vehicleNo" class="field text medium"/>
	<form:errors path="vehicleNo" cssClass="error"></form:errors>
</div>
</li>
...
<div>
	<input type="radio" name="noOfPerson" value="alone" onclick="showNoOfPersons('personCount',true);">Alone
	<input type="radio" name="noOfPerson" value="multiple" onclick="showNoOfPersons('personCount',false);">Multiple
	&nbsp;&nbsp;No. of People<form:input path="noOfPeople" name="personCount" readonly="true" id="personCount" type="text"/>
</div>
</li>
...
<li class="buttons ">
<div>
    <input class="btTxt submit" type="submit" name="save" id="save" onclick="return submitUpdateVisitorLogs();"  value="Update"/>
</div>
</li>
