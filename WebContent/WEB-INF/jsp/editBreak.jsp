...
<form:form commandName="breaks" action="saveOrUpdateBreak.htm?Id=${id}" method="POST" class="wufoo leftLabel page">
...
<form:select path="employee_no"  style="width: 150px;" class="field select medium">  
	    <form:option value="0">--Select--</form:option>
	    	<c:forEach items="${employeeList}" var="employee">
	    		<form:option value="${employee.employeeNo}" label="${employee.firstName} ${employee.lastName}">${employee.firstName} ${employee.lastName}</form:option>
    		</c:forEach>
    </form:select>
...
<div>
<form:input path="title" class="field text medium" value="${breakObj.title}"/>
</div>
...
<div>
<form:input type="text" id="breakdate" name="breakdate" path="date" readonly="true" size="15" class="field text medium"/>
	<img
	id="dobImage1" src="images/cal.gif" alt="cal"
	onClick="setYears(1955, 2015);showCalender(this, 'breakdate',-350,-270);"></img>
...
<div>
 <form:select path="duration"  style="width: 150px;" class="field select medium"> 
    	<c:choose>
    	<c:when test="${breakObj.duration==1}">
   		<form:option selected="selected" value="1">--Select--</form:option>
    	</c:when>
    	<c:otherwise>
    	<form:option selected="selected" value="${breakObj.duration}">${breakObj.duration}</form:option>
    	</c:otherwise>
    	</c:choose>
...
<div> 
<form:textarea path="description" name="description" class="field textarea medium"></form:textarea>
</div>
</li>  
...
<div>
 <form:select path="status"  style="width: 150px;" class="field select medium"> 
	    <form:option value="1">--Select--</form:option>
		    ...
		    <c:choose>
		    <c:when test="${breakObj.status==statusListVar.id}">
		    <form:option selected="selected" value="${statusListVar.id}">${statusListVar.status_name}</form:option>		    
		    </c:when>
		    <c:otherwise>
...
<div>
<form:input type="text" id="compensationDate" name="compensationDate" path="compensation_date"  readonly="true" size="15" class="field text medium"/>
	<img
	id="dobImage1" src="images/cal.gif" alt="cal"
