...
private SessionFactory sessionFactory;
	...
	public List<Countries> getCountriesList() {
		...
		return hibernateTemplate.find("from Countries");
	}
	@Override
	public List<Countries> getCountryById(int countryid) {
		return hibernateTemplate.find("from Countries where countryId="+countryid);
		
	/*	Session session = sessionFactory.openSession();
		String query="Select Country from Countries where  countryId="+countryid;
		List<String> countries =  session.createQuery(query).list();
		session.close();
		return countries;*/
	...
	public void saveCountries() {
		...
		String[] s_a = new String[253];
		s_a[0]="--Select--";
		s_a[1]="Badakhshan|Badghis|Baghlan|Balkh|Bamian|Farah|Faryab|Ghazni|Ghowr|Helmand|Herat|Jowzjan|Kabol|Kandahar|Kapisa|Konar|Kondoz|Laghman|Lowgar|Nangarhar|Nimruz|Oruzgan|Paktia|Paktika|Parvan|Samangan|Sar-e Pol|Takhar|Vardak|Zabol";
		//String[] a=s_a[1].split("\\|");
		...
		for(int i=0;i<((country_arr.length)-1);i++)
		{
			Countries countries=new Countries();
			countries.setCountry(country_arr[i]);
			hibernateTemplate.save(countries);
		 List<Countries> aaaa=hibernateTemplate.find("from Countries where country='"+country_arr[i]+"'");
		 String[] temp=s_a[i+1].split("\\|");
		 int tempCId= aaaa.get(0).getCountryId();
		 for(String obj:temp)
		 {
			 States states=new States();
			 states.setStateName(obj);
			 states.setCountryId(tempCId);
			 hibernateTemplate.save(states);
		 }
		}
